...
class DashboardController implements RouteInterface
{
  ...
  {
    $routingService->getRouter()->get("/", function () use ($routingService) {
    ...
    if (!AuthService::isLoggedIn()) {
      ...
      $routingService->getRouter()->get_post("/dashboard/lib/mods/?/version/?", function ($mod, $versionId) use ($routingService) {
        if (isset($_POST['action'])) {
          ...
          if ($_POST['action'] == "delete") {
            ModpackService::deleteModVersion($mod, $versionId);
          }
      ...
      $routingService->getRouter()->get_post("/dashboard/about", function () use ($routingService) {

        DashboardController::render($routingService, "/dashboard/about/index.twig", ["license" => file_get_contents(Solder::getPath() . "/LICENSE")]);
      });
      $routingService->getRouter()->get_post("/dashboard/update", function () use ($routingService) {
        $theSystemsObj = Solder::getTheSystemsObj();

        DashboardController::render($routingService, "/dashboard/about/update.twig", ["HMCSW" => ["check" => $theSystemsObj->checkVersion(),
          "version" => $theSystemsObj->version,
          "lastCheck" => $theSystemsObj->lastCheck]]);
      });
      $routingService->getRouter()->get_post("/dashboard/settings", function () use ($routingService) {
        DashboardController::render($routingService, "/dashboard/about/settings.twig", []);
      });
      $routingService->getRouter()->any("/solder", function () use ($routingService) {
        Solder::getTheSystemsObj()->softwarePage();
      });
    }
  }
